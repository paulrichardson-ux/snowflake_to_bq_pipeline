...
SOURCE_DIR="./work_item_budget_vs_actual_pipeline/work_item_budget_vs_actual_sync_daily" # Relative path from where script is run (likely workspace root)
ENTRY_POINT="sync_daily_incremental"
RUNTIME="python311" # Match the runtime from the other script
MEMORY="2048MB" # Increased memory to prevent OOM issues
TIMEOUT="900s"  # Increased timeout to 15 minutes to handle large datasets
