...
def sync_full_work_item_budget_vs_actual(request):
    """Cloud Function for FULL sync of WORK_ITEM_BUDGET_VS_ACTUAL from Snowflake to BigQuery."""
    project_id = os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("GCP_PROJECT")
    ...
    try:
        ...
        temp_table_name = f"temp_work_item_budget_vs_actual_full_{uuid.uuid4().hex}"
        ...
        try:
            while True:
                ...
                """
                print(f"Fetching FULL batch from Snowflake WORK_ITEM_BUDGET_VS_ACTUAL: offset={offset}, batch_size={batch_size}")
                cs.execute(query)
                sf_rows = cs.fetchall()
        ...
        if total_rows_fetched == 0:
            print("No rows found in Snowflake WORK_ITEM_BUDGET_VS_ACTUAL table.")
        ...
        print("Work Item Budget vs Actual FULL sync completed successfully.")
        return f"Full sync finished. Total rows processed: {total_rows_fetched}", 200

    except Exception as e:
        print(f"Error during Work Item Budget vs Actual FULL sync: {e}")
