...
def get_bq_schema_for_dimn_tenant_team():
    """
    Define BigQuery schema for DIMN_TENANT_TEAM table
    Based on the CSV structure: TENANT_TEAM_ID,TENANT_TEAM_NAME,PARENT_TENANT_TEAM_ID,PARENT_TENANT_TEAM_NAME,ACCOUNT_ID,ACCOUNT_NAME
    """
    return [
        bigquery.SchemaField("TENANT_TEAM_ID", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("TENANT_TEAM_NAME", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("PARENT_TENANT_TEAM_ID", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("PARENT_TENANT_TEAM_NAME", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("ACCOUNT_ID", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("ACCOUNT_NAME", "STRING", mode="NULLABLE"),
    ]

@functions_framework.http
def sync_full_tenant_team_dimension(request):
    """
    Cloud Function to sync DIMN_TENANT_TEAM from Snowflake to BigQuery
    Performs full table replacement (truncate and insert)
    """
    try:
        logger.info("Starting TENANT_TEAM dimension full sync")
        ...
        schema = get_bq_schema_for_dimn_tenant_team()
        ...
        if rows:
            ...
            for row in rows:
                tenant_team_record = {
                    'TENANT_TEAM_ID': row[0],
                    'TENANT_TEAM_NAME': row[1],
                    'PARENT_TENANT_TEAM_ID': row[2],
                    'PARENT_TENANT_TEAM_NAME': row[3],
                    'ACCOUNT_ID': row[4],
                    'ACCOUNT_NAME': row[5]
                }
                tenant_team_data.append(tenant_team_record)
