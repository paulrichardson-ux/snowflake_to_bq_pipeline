...
def get_bq_schema_from_snowflake(sf_creds):
    """Connects to Snowflake and derives a BigQuery schema from DIMN_CLIENT."""
...
def sync_client_dimension_full(request):
    """Entry point for CLIENT_DIMENSION full sync Cloud Function - replaces table to exactly mirror Snowflake"""
    project_id = os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("GCP_PROJECT")
    dataset_id = os.getenv("BQ_DATASET")
    target_table_id = f"{project_id}.{dataset_id}.CLIENT_DIMENSION"

    if not project_id or not dataset_id:
        raise ValueError("GOOGLE_CLOUD_PROJECT and BQ_DATASET environment variables must be set.")
    print(f"Starting CLIENT_DIMENSION full sync for {target_table_id}")
    ...
    try:
        ...
        temp_table_name = f"temp_client_dimension_sync_{uuid.uuid4().hex}"
        ...
        cs = conn.cursor()
        offset = 0
        batch_size = 1000  # Reasonable batch size for client dimension data
        ...
        try:
            while True:
                ...
                """
                print(f"Fetching batch from Snowflake DIMN_CLIENT: offset={offset}, batch_size={batch_size}")
                cs.execute(query)
                sf_rows = cs.fetchall()
                ...
                if not sf_rows:
                    print("No more rows found in Snowflake DIMN_CLIENT.")
                    break
        ...
        print(f"Table replacement completed. Rows inserted: {query_job.num_dml_affected_rows}")

        # Delete temporary table after successful replacement
        print(f"Deleting temporary table: {temp_table_id}")
        bq_client.delete_table(temp_table_id, not_found_ok=True)

        print("CLIENT_DIMENSION full sync completed successfully.")
        return f"CLIENT_DIMENSION sync finished. Total rows processed: {total_rows_fetched}", 200

    except Exception as e:
        print(f"Error during CLIENT_DIMENSION sync: {e}")
