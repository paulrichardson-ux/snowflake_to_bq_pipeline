...
# Deploy USER_DIMENSION sync Cloud Function
FUNCTION_NAME="user-dimension-sync-daily"
REGION="us-central1"
PROJECT_ID="red-octane-444308-f4"
DATASET_ID="karbon_data"

echo "Deploying USER_DIMENSION sync function..."

gcloud functions deploy $FUNCTION_NAME \
  --gen2 \
  --region=$REGION \
  --runtime=python311 \
  --source=./user_dimension_sync_daily \
  --entry-point=sync_user_dimension_full \
  --trigger-http \
...
if [ $? -eq 0 ]; then
    echo "✅ USER_DIMENSION sync function deployed successfully!"
