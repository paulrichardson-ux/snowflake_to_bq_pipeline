...
def get_bq_schema_for_dimn_tenant_team_member():
    """Returns the BigQuery schema for DIMN_TENANT_TEAM_MEMBER table."""
    return [
        bigquery.SchemaField("TENANT_TEAM_MEMBER_ID", "STRING", mode="NULLABLE"),
...
def sync_full_tenant_team_member_dimension(request):
    """Entry point for TENANT_TEAM_MEMBER_DIMENSION full sync Cloud Function"""
    project_id = os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("GCP_PROJECT")
    ...
    try:
        ...
        # Use predefined schema for DIMN_TENANT_TEAM_MEMBER
        bq_schema = get_bq_schema_for_dimn_tenant_team_member()
        ...
        cs = conn.cursor()
        offset = 0
        batch_size = 1000  # Reasonable batch size for tenant team member data
