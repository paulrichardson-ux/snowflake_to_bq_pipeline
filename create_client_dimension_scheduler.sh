#!/bin/bash

# Create Cloud Scheduler job for CLIENT_DIMENSION daily sync
JOB_NAME="client-dimension-daily-sync"
FUNCTION_NAME="client-dimension-sync-daily"
REGION="us-central1"
PROJECT_ID="red-octane-444308-f4"
SCHEDULE="0 6 * * *"  # Daily at 6 AM UTC
TIMEZONE="UTC"

echo "Creating Cloud Scheduler job for CLIENT_DIMENSION daily sync..."

gcloud scheduler jobs create http $JOB_NAME \
  --location=$REGION \
  --schedule="$SCHEDULE" \
  --time-zone=$TIMEZONE \
  --uri="https://$REGION-$PROJECT_ID.cloudfunctions.net/$FUNCTION_NAME" \
  --http-method=POST \
  --headers="Content-Type=application/json" \
  --message-body='{"source": "scheduler"}' \
  --description="Daily sync of CLIENT_DIMENSION from Snowflake to BigQuery"

if [ $? -eq 0 ]; then
    echo "✅ Cloud Scheduler job '$JOB_NAME' created successfully!"
    echo "Schedule: Daily at 6 AM UTC"
    echo "Function: $FUNCTION_NAME"
else
    echo "❌ Scheduler job creation failed!"
    exit 1
fi

echo ""
echo "To manually trigger the job:"
echo "gcloud scheduler jobs run $JOB_NAME --location=$REGION" 