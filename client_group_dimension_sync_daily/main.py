...
def get_bq_schema_from_snowflake(sf_creds):
    """
    Auto-detect BigQuery schema from Snowflake DIMN_CLIENT_GROUP table
    """
    conn = snowflake.connector.connect(**sf_creds)
    cursor = conn.cursor()
    
    # Get column information from Snowflake
    cursor.execute("DESCRIBE TABLE DIMN_CLIENT_GROUP")
...
def sync_client_group_dimension_full(request):
    """
    ...
    """
    try:
        logger.info("Starting CLIENT_GROUP dimension daily sync (full replacement)")
        ...
        query = "SELECT * FROM DIMN_CLIENT_GROUP ORDER BY CLIENT_GROUP_NAME, CLIENT"
        ...
        if rows:
            ...
            for row in rows:
                ...
                client_group_data.append(record)
