...
def get_bq_schema_from_snowflake(sf_creds):
    """Connects to Snowflake and derives a BigQuery schema from DIMN_USER."""
...
def sync_user_dimension_full(request):
    """Entry point for USER_DIMENSION full sync Cloud Function - replaces table to exactly mirror Snowflake"""
    project_id = os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("GCP_PROJECT")
    ...
    try:
        ...
        cs = conn.cursor()
        offset = 0
        batch_size = 1000  # Reasonable batch size for user dimension data
