...
def get_bq_schema_for_dimn_user():
    """Returns the BigQuery schema for DIMN_USER table."""
    return [
        bigquery.SchemaField("USER_NAME", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("USER_ID", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("USER_JOB_TITLE", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("USER_EMAIL_ADDRESS", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("CREATED_DATE", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("ACTIVATED_DATE", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("EXPECTED_BILLABLE_MINUTES", "INT64", mode="NULLABLE"),
        bigquery.SchemaField("EXPECTED_NONBILLABLE_MINUTES", "INT64", mode="NULLABLE"),
        bigquery.SchemaField("STATUS", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("IS_SUPPORT_USER", "BOOL", mode="NULLABLE"),
        bigquery.SchemaField("ACCOUNT_ID", "STRING", mode="NULLABLE"),
        bigquery.SchemaField("ACCOUNT_NAME", "STRING", mode="NULLABLE"),
    ]

@functions_framework.http
def sync_full_user_dimension(request):
    """Entry point for USER_DIMENSION full sync Cloud Function"""
    project_id = os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("GCP_PROJECT")
    dataset_id = os.getenv("BQ_DATASET")
    target_table_id = f"{project_id}.{dataset_id}.USER_DIMENSION"

    if not project_id or not dataset_id:
        raise ValueError("GOOGLE_CLOUD_PROJECT and BQ_DATASET environment variables must be set.")
    
    print(f"Starting USER_DIMENSION full sync for {target_table_id}")
    ...
    try:
        ...
        temp_table_name = f"temp_user_dimension_sync_{uuid.uuid4().hex}"
        temp_table_id = f"{project_id}.{dataset_id}.{temp_table_name}"

        # Get schema and create the single temporary table
        print(f"Preparing temporary table: {temp_table_id}")
        
        # Use predefined schema for DIMN_USER
        bq_schema = get_bq_schema_for_dimn_user()
        ...
        try:
            ...
            print(f"Target table {target_table_id} already exists.")
        except NotFound:
        ...
        cs = conn.cursor()
        offset = 0
        batch_size = 1000  # Reasonable batch size for user data
        ...
        try:
            while True:
                query = f"""
                    SELECT *
                    FROM {sf_creds["schema"]}.DIMN_USER
                    ORDER BY USER_ID
                    LIMIT {batch_size} OFFSET {offset}
                """
                print(f"Fetching batch from Snowflake DIMN_USER: offset={offset}, batch_size={batch_size}")
                cs.execute(query)
                sf_rows = cs.fetchall()
                ...
                if not sf_rows:
                    print("No more rows found in Snowflake DIMN_USER.")
                    break
                ...
                for row in sf_rows:
                    ...
                    for idx, col_name in enumerate(sf_cols):
                        ...
                        elif col_name == "IS_SUPPORT_USER" and value is not None:
                            # Convert string boolean to actual boolean
                            row_dict[col_name] = str(value).upper() in ('TRUE', '1', 'YES')
                        else:
        ...
        if total_rows_fetched == 0:
            print("No rows found in Snowflake DIMN_USER table.")
        ...
        return f"Successfully synced {total_rows_fetched} rows to USER_DIMENSION", 200

    except Exception as e:
        print(f"❌ Error in USER_DIMENSION sync: {e}")
